    ...
    void Update()
    {
        if (gps_ok)
        {
            ...
            */
            ...
            //4.865520359688291, -74.04014054050185

            double distanceBetween = distance((double)currLoc.lat, (double)currLoc.lon, (double)4.865520359688291, (double)-74.04014054050185, 'K');

            debugTxt.text += "\nDistance: " + distanceBetween;

            if (distanceBetween<0.01)
            {
                if(createdEsfera==false) 
                { 
                    createdEsfera = true;
                    Instantiate(ESFERA, new Vector3(), Quaternion.identity);
