...
namespace Mapbox.Unity.MeshGeneration.Data
{
	...
	public class UnityTile : MonoBehaviour
	{
        ...
        public void SetRasterData(byte[] data, bool useMipMap = true, bool useCompression = false)
        {
            ...
            {
                // Validación 1: datos nulos o vacíos
                if (data == null || data.Length == 0)
                {
                    Debug.LogError("SetRasterData error: Raster data is null or empty.");
                    MeshRenderer.material.mainTexture = null;
                    return;
                }

                // Crea una textura temporal para cargar los datos y validar dimensiones
                Texture2D tempTex = new Texture2D(2, 2);
                if (!tempTex.LoadImage(data))
                {
                    Debug.LogError("SetRasterData error: Failed to load image from byte array.");
                    MeshRenderer.material.mainTexture = null;
                    return;
                }

                // Validación 2: dimensiones válidas
                if (tempTex.width <= 0 || tempTex.height <= 0)
                {
                    Debug.LogError($"SetRasterData error: Invalid texture dimensions: {tempTex.width}x{tempTex.height}");
                    MeshRenderer.material.mainTexture = null;
                    return;
                }

                // Si no hay textura previa, la creamos
                if (_rasterData == null)
                {
                    _rasterData = new Texture2D(tempTex.width, tempTex.height, TextureFormat.RGB24, useMipMap);
                    _rasterData.wrapMode = TextureWrapMode.Clamp;
                }
                else if (_rasterData.width != tempTex.width || _rasterData.height != tempTex.height)
                {
                    // Si las dimensiones no coinciden, creamos una nueva textura
                    Destroy(_rasterData);
                    _rasterData = new Texture2D(tempTex.width, tempTex.height, TextureFormat.RGB24, useMipMap);
                    _rasterData.wrapMode = TextureWrapMode.Clamp;
                }

                _rasterData.SetPixels(tempTex.GetPixels());
                _rasterData.Apply();
